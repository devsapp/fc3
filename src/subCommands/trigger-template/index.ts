...
export default class ComponentDemo {
  ...
  public async http() {
    ...
    logger.write(`
      ...
      You could user fc component invoke method and specify the event.
      E.g: [s projectName invoke --event-file  ${path.join(
        './event-template/',
        'http-parameter.json',
      )}]
   ...
   * @returns
   */
  public async tablestore() {
    await fse.mkdirs('./event-template/');
    const templateData = await fse.readFileSync(
      path.join(__dirname, 'event-template/tablestore.json'),
    );
  ...
  public async mns() {
    ...
    const cicdPlatform: any = await inquirer.prompt([
      {
        ...
        choices: [
          {
            name: 'When the event format is set to STREAM, the message does not contain message attributes (MessageAttributes)',
            value: '0',
          },
          {
            name: 'When the event format is set to STREAM and the message contains message attributes (MessageAttributes)',
            value: '1',
          },
          {
            name: 'When the event format is set to JSON and the message does not contain message attributes (MessageAttributes)',
            value: '2',
          },
          {
            name: 'When the event format is set to JSON and the message contains message attributes (MessageAttributes)',
            value: '3',
          },
        ],
      },
    ...
    switch (cicdPlatform.platform) {
      case '0':
        ...
        break;
      case '1':
        templateData = await fse.readFileSync(
          path.join(__dirname, 'event-template/mns-stream.json'),
        );
      ...
      case '2':
        templateData = await fse.readFileSync(
          path.join(__dirname, 'event-template/mns-without-MessageAttributes.json'),
        );
      ...
      case '3':
        templateData = await fse.readFileSync(
          path.join(__dirname, 'event-template/mns-with-MessageAttributes.json.json'),
        );
  ...
  public async cdn() {
    ...
    switch (cicdPlatform.platform) {
      case '0':
        templateData = await fse.readFileSync(
          path.join(__dirname, 'event-template/cdn-LogFileCreated.json'),
        );
      ...
      case '1':
        templateData = await fse.readFileSync(
          path.join(__dirname, 'event-template/cdn-CachedObjectsRefreshed.json'),
        );
      ...
      case '4':
        templateData = await fse.readFileSync(
          path.join(__dirname, 'event-template/cdn-CdnDomainStarted.json'),
        );
      ...
      case '6':
        templateData = await fse.readFileSync(
          path.join(__dirname, 'event-template/cdn-CdnDomainAdded.json'),
        );

...
export class BaseLocalStart extends BaseLocal {
  ...
  async getLocalStartCmdStr(): Promise<string> {
    ...
    const dockerCmdStr = `docker run -i --name ${this.getContainerName()} --platform linux/amd64 --rm -p ${port}:${this.getCaPort()} --memory=${this.getMemorySize()}m ${mntStr} ${envStr} ${nasStr} ${layerStr} ${image} --http --server`;
  ...
  async setupHttpProxy() {
    ...
    proxy.on('proxyRes', async (proxyRes) => {
      if (!this.isDebug()) {
        if (proxyRes.headers['x-fc-log-result']) {
